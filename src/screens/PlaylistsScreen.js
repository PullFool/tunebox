...
export default function PlaylistsScreen() {
  const {
    playlists, createPlaylist, deletePlaylist, playSong,
    currentSong, isPlaying, addToPlaylist, removeFromPlaylist, library,
  } = usePlayer();
  ...
  if (selectedPlaylist) {
    ...
    const isSongAdded = (url) => playlist.songs.some(s => s.url === url);
      ...
      <View style={styles.container}>
        <View style={styles.detailHeader}>
          ...
          <View style={{ flex: 1, marginLeft: 12 }}>
            <Text style={styles.detailTitle}>{playlist.name}</Text>
            <Text style={styles.detailCount}>{playlist.songs.length} songs</Text>
          </View>
          <TouchableOpacity style={styles.addSongsBtn} onPress={() => setShowAddSongs(true)}>
            <Ionicons name="add" size={18} color="#fff" />
            <Text style={{ color: '#fff', fontSize: 12, marginLeft: 4 }}>Add</Text>
          </TouchableOpacity>
          {playlist.songs.length > 0 && (
            <TouchableOpacity
              style={styles.playAllBtn}
              onPress={() => playSong(playlist.songs, 0)}
            >
        ...
        {playlist.songs.length === 0 ? (
          <View style={styles.empty}>
            <Ionicons name="musical-notes" size={50} color="#6a6a6a" />
            <Text style={styles.emptyTitle}>Empty playlist</Text>
            <Text style={styles.emptyText}>Tap "Add" to browse your library</Text>
          </View>
        ) : (
          <FlatList
            data={playlist.songs}
            ...
            renderItem={({ item, index }) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => playSong(playlist.songs, index)}
                >
                  ...
                  <TouchableOpacity onPress={() => removeFromPlaylist(playlist.id, item.url)}>
        ...
        <Modal visible={showAddSongs} animationType="slide" transparent>
          <View style={styles.modalOverlay}>
            <View style={styles.modal}>
              ...
              {library.length === 0 ? (
                <Text style={styles.emptyText}>Play some songs first to build your library!</Text>
              ) : (
                <FlatList
                  data={library}
                  ...
                  renderItem={({ item }) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => added ? removeFromPlaylist(playlist.id, item.url) : addToPlaylist(playlist.id, item)}
                      >
                        <View style={{ flex: 1 }}>
                          <Text style={{ color: '#fff', fontSize: 13 }} numberOfLines={1}>{item.title}</Text>
                          <Text style={{ color: '#b3b3b3', fontSize: 11 }}>{item.artist}</Text>
                        </View>
                        <View style={[styles.checkCircle, added && styles.checkCircleActive]}>
                          <Ionicons name={added ? 'checkmark' : 'add'} size={16} color={added ? '#000' : '#b3b3b3'} />
                        </View>
                      </TouchableOpacity>
                    );
                  }}
              ...
              <TouchableOpacity style={styles.doneBtn} onPress={() => setShowAddSongs(false)}>
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity style={styles.createBtn} onPress={() => setShowCreate(true)}>
        ...
        <Text style={{ color: '#000', fontWeight: '600', marginLeft: 4 }}>New Playlist</Text>
      </TouchableOpacity>

      {playlists.length === 0 ? (
        <View style={styles.empty}>
          <Ionicons name="list" size={60} color="#6a6a6a" />
          <Text style={styles.emptyTitle}>No playlists yet</Text>
          <Text style={styles.emptyText}>Create one and add songs</Text>
        </View>
      ) : (
        <FlatList
          data={playlists}
            ...
            >
              <View style={styles.playlistArt}>
                {item.songs.length > 0 && item.songs[0].cover ? (
                  <Image source={{ uri: item.songs[0].cover }} style={{ width: '100%', height: '100%' }} />
                ) : (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modal}>
            ...
            <View style={{ flexDirection: 'row', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <TouchableOpacity style={styles.doneBtn} onPress={handleCreate}>
...
const styles = StyleSheet.create({
  ...
  createBtn: {
    backgroundColor: '#1DB954', borderRadius: 20, flexDirection: 'row',
  ...
  playlistCard: { flex: 1, backgroundColor: '#181818', borderRadius: 8, padding: 12 },
  playlistArt: {
    width: '100%', aspectRatio: 1, borderRadius: 6, backgroundColor: '#282828',
    alignItems: 'center', justifyContent: 'center', marginBottom: 8, overflow: 'hidden',
  },
  playlistName: { color: '#fff', fontSize: 14, fontWeight: '600' },
  ...
  detailCount: { color: '#b3b3b3', fontSize: 12 },
  addSongsBtn: {
    flexDirection: 'row', alignItems: 'center', borderWidth: 1,
    borderColor: '#555', borderRadius: 16, paddingHorizontal: 12, paddingVertical: 6, marginRight: 8,
  },
  playAllBtn: {
    backgroundColor: '#1DB954', borderRadius: 20, width: 36, height: 36,
  ...
  modalInput: {
    backgroundColor: '#121212', borderRadius: 6, padding: 12,
    color: '#fff', fontSize: 14, borderWidth: 1, borderColor: '#333', marginBottom: 16,
  },
  cancelBtn: {
  ...
  doneBtn: {
    ...
    paddingHorizontal: 20, paddingVertical: 10, marginTop: 12, alignItems: 'center',
  },
  ...
  checkCircleActive: { backgroundColor: '#1DB954', borderColor: '#1DB954' },
});
